...
class TransformHtmlProceedingsToXml(object):
    ...
    def get_political_groups(self, tree, id):
        ...
        for i in political_groups:
            ...
            p_group = info[2]
            p_group_role = info[3]
            output.append({'id': id, 'm_state': m_state, 's_date': s_date, 'e_date': e_date, 'p_group': p_group, 'p_group_role': p_group_role})
        return output
    ...
    def get_national_parties(self, tree, id):
        ...
        for i in political_groups:
            ...
            if info[1] == '...':
                ...
                e_date = self.parse_date(info[1], "%d.%m.%Y")
            n_party = info[2]
            output.append({'id': id,'s_date': s_date, 'e_date': e_date, 'n_party': n_party})
        return output
    ...
    def extract_info(self, infile):
        ...
        self.political_groups = self.political_groups + self.get_political_groups(tree, id)
        self.national_parties = self.national_parties + self.get_national_parties(tree, id)
        pass
    ...
    def serialize_dict_of_dicts(self, dict_of_dicts, ofile_name):
        ...
        df.to_csv(opath, sep='\t', mode='w', encoding='utf-8', index_label='id')
        pass
    ...
    def main(self):
        ...
        for infile in self.infiles:
            ...
            if self.date is None:
                self.date = datetime.datetime.fromtimestamp(os.path.getmtime(infile)).date()
            self.extract_info(infile)
            self.n_proceedings += 1
        self.serialize_dict_of_dicts(self.meps, 'meps.csv')
        self.serialize_list_of_dicts(self.political_groups, 'political_groups.csv', ['id', 'm_state', 's_date', 'e_date', 'p_group', 'p_group_role'])
        self.serialize_list_of_dicts(self.national_parties, 'national_parties.csv', ['id', 's_date', 'e_date', 'n_party'])
        pass
